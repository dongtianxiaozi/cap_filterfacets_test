...
export class ActivitiesRepository {
	...
	async getUnitsOfActivity(
		...
		try {
			...
			if (resultActivities.length !== 1) return Left(new UnexpectedError());
			...
			const resultActivitiesPhases: OrderService.IUnits[] = await this.dbDatasource.executeOrThrow({
				SELECT: {
					...
					where: [{ ref: ['ID'] }, '=', { val: resultActivities[0].toUnit_ID }],
				},
			});
			return Right(new QueryResult(resultActivitiesPhases));
		} catch (e) {
			this.logger.w(
				ActivitiesRepository.name,
				() => `error Units Unit of Activity: ${JSON.stringify(Object.getOwnPropertyNames(e))}`
			);
