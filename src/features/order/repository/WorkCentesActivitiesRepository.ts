...
export class WorkCentesActivitiesRepository {
	...
	async getGrantedTypesWithPhase(
		...
		this.logger.d(
			WorkCentesActivitiesRepository.name,
			() => `get GrantedTypes with phases=${phases} in WorkCenters_Activities`
		);
		try {
			let _where = [];
			phases.forEach((element) => {
				if (_where.length !== 0) _where.push('or');
			...
			if (resultActivitiesPhases.length === 0) return Right(new QueryResult([]));
			...
			const resultWorkCentersActivities: OrderService.IWorkCenters_Activities[] = await this.dbDatasource.executeOrThrow(
				{
					SELECT: {
						...
						where: _where,
						columns: [{ ref: ['toGrantedType_ID'] }],
					},
				}
			);

			if (resultWorkCentersActivities.length === 0) return Right(new QueryResult([]));
			...
			this.logger.w(
				WorkCentesActivitiesRepository.name,
				() => `error get GrantedTypes with phases: ${JSON.stringify(Object.getOwnPropertyNames(e))}`
			);
	...
	async getActivityPhasesWithId(
		...
		try {
			...
			this.logger.w(
				WorkCentesActivitiesRepository.name,
				() => `error getting Activity_Phase: ${JSON.stringify(Object.getOwnPropertyNames(e))}`
			);
			return Left(new UnexpectedError());
		}
	}
