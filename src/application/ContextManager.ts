...
export class ContextManager {
	...
	startContext(fn: () => void) {
		ContextManager.getContext().run(fn);
	}

	async startContextWithPromise(req, fn: () => Promise<any>): Promise<any> {
		return ContextManager.getContext().runAndReturn(() => {
	...
	private static getContext(): cls.Namespace {
	...
	getEnvironment(): IEnvironment {
		return ContextManager.getContext().get(ContextManager.ENVIRONMENT);
	}

	setEnvironment(environment: IEnvironment) {
		ContextManager.getContext().set(ContextManager.ENVIRONMENT, environment);
	}

	getCurrentTransaction(): Transaction {
		return ContextManager.getContext().get(ContextManager.CURRRENT_TRANSACTION);
	}

	setCurrentTransaction(tx: Transaction) {
		ContextManager.getContext().set(ContextManager.CURRRENT_TRANSACTION, tx);
	}
}
